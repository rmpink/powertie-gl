...
bool Font::LoadFromBitmap( std::string path ) {
	...
	if ( LoadFromFile( path ) ) {
		...
		for ( int row = 0; row < FONT_CELL_ROWS; ++row ) {

			for ( int col = 0; col < FONT_CELL_COLS; ++col ) {
		...
		for ( int i = 0; i < ( FONT_CELL_COLS * FONT_CELL_ROWS ); ++i ) {
	...
	if ( success ) {
		...
		if ( !error ) {
			...
			if ( !error ) {
				...
				for ( int clipY = 0; clipY < FONT_CELL_ROWS; ++clipY ) {

					for ( int clipX = 0; clipX < FONT_CELL_COLS; ++clipX ) {
